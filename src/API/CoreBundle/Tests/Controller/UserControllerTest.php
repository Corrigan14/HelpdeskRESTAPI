...
class UserControllerTest extends ApiTestCase
{
    ...
    {
        $user = $this->getClient(true)->request('POST', $this->getBaseUrl(), [
            'username' => 'user', 'password' => 'user', 'email' => 'user@user.sk',
    ...
    public function testUpdateSingleSuccess()
    {
        ...
        $this->getClient(true)->request('PUT', $this->getBaseUrl() . '/' . $entity->getId(),
            [
                'email' => 'user@user.sk', 'username' => 'user',
                'detail_data' => ['name' => 'returned name', 'surname' => 'returned surname'],
            ],
            [], ['Authorization' => 'Bearer ' . $this->adminToken, 'HTTP_AUTHORIZATION' => 'Bearer ' . $this->adminToken]);
        $this->assertEquals(StatusCodesHelper::SUCCESSFUL_CODE, $this->getClient()->getResponse()->getStatusCode());
    }
